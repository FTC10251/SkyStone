 ...
 */
@Autonomous(name = "SkyStone Auto Test (BLUE)", group = "HDrive")
public class SkyStoneAutonomousUpdatedBlue extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if(blockPosition == 0) { //Right
            ...
            turnInCircleProfiled(16,2,-1,30, .6,.1,.6,0,10,0);
            leftIntakeServo.setPosition(.55);
            rightIntakeServo.setPosition(.55);
            ThreadSleepUpdated(300);
            turnInCircleProfiled(7,2,1,60, -.1,-.1,-.5,2,4,0);
            pickUpSkystone1();

            //Turn towards foundation
            encoderDriveProfiled(-.1, -.1, -.8, 56, 2, 15, 90, true);
            leftIntakeMotor.setPower(0);
            rightIntakeMotor.setPower(0);
            pickUpSkystone2();
        } else if(blockPosition == 1) { //Mid
            encoderDriveProfiled(.2, .1, .5, 32, 1, 16, 0, true);
            leftIntakeServo.setPosition(.55);
            rightIntakeServo.setPosition(.55);
            ...
            //Turn towards foundation
            encoderDriveProfiled(-.1, -.1, -.8, 60, 2, 15, 90, true);
            leftIntakeMotor.setPower(0);
            rightIntakeMotor.setPower(0);
            pickUpSkystone2();
        } else { //Left
            encoderDriveProfiled(.2,.6,.6,10,1,6,0,false);
            turnInCircleProfiled(20,2,1,25, .6,.1,.6,0,10,0);
            leftIntakeServo.setPosition(.55);
            rightIntakeServo.setPosition(.55);
            ThreadSleepUpdated(300);
            turnInCircleProfiled(7,2,1,115, -.1,-.4,-.4,2,4,0);
            pickUpSkystone1();
            encoderDriveProfiled(-.4, -.1, -.8, 60, 2, 15, 90, true);
            leftIntakeMotor.setPower(0);
            rightIntakeMotor.setPower(0);
            pickUpSkystone2();
            ThreadSleepUpdated(100);
        ...
        if(blockPosition == 0) {
            moveToNextBlock(.1, .05, .8, 82, 2, 35, 90, 68, true);
            moveIntakeAndArm2();
            ThreadSleepUpdated(100);
            turnInCircleProfiled(25,2,1,30, .5,.1,.4,2,4,0);
            leftIntakeServo.setPosition(.55);
            rightIntakeServo.setPosition(.55);
            ThreadSleepUpdated(400);
            turnInCircleProfiled(25,2,1,30, -.5,-.3,-.4,2,4,-.3);
        } else if(blockPosition == 1) {
            moveToNextBlock(.1, .05, .8, 75, 2, 35, 90, 70, true);
            moveIntakeAndArm2();
            ThreadSleepUpdated(100);
            turnInCircleProfiled(25,2,1,35, .5,.1,.4,2,4,0);
            ThreadSleepUpdated(500);
            leftIntakeServo.setPosition(.55);
            rightIntakeServo.setPosition(.55);
            ThreadSleepUpdated(400);
            turnInCircleProfiled(25,2,1,35, -.5,-.3,-.4,2,4,-.3);
        } else {
            moveToNextBlock(.1, .05, .8, 73, 2, 35, 90, 70, true);
            moveIntakeAndArm2();
            ThreadSleepUpdated(100);
            turnInCircleProfiled(25,2,1,35, .5,.1,.4,2,4,0);
            ThreadSleepUpdated(500);
        ...
        pickUpSkystone1();//
        if(blockPosition == 0) {
            encoderDriveProfiled(-.3, -.1, -.8, 57, 2, 15, 90, true);
        } else if(blockPosition == 1) {
            encoderDriveProfiled(-.3, -.1, -.8, 45, 2, 15, 90, true);
        } else {
            encoderDriveProfiled(-.3, -.1, -.8, 38, 2, 15, 90, true);
        }

        scoreBlockAtEnd();
        ThreadSleepUpdated(500);
        encoderDriveProfiled(.1,.1,.5,30, 2,15,90,true);
    ...
    public void turnInCircle(double radius, double velocity, double turnDirection, double rotations, double timeout) {
        ...
        while (opModeIsActive() && !isStopRequested() && !finishedMotion) {
            ...
            double angleDifference = extraClasses.angleDistance(startingAngle, currentAngle);
    ...
    public void lignUpWithFoundation() {
        ...
        while (!touchSensor.isPressed() && opModeIsActive() && !isStopRequested()) {
            ...
            double goalAngle = 175; //Just the starting angle I think
            ...
            angleAdjustPower = angleError / 40;
            distance = rangeSensorBack.getDistance(DistanceUnit.CM);
            if(distance > 30) {
                distance = 30;
            }
            double sidePower = -distance/230 - .1;
            telemetry.addData("Distance", distance);
            telemetry.update();
            leftMotor.setPower(sidePower + angleAdjustPower);
            leftMotor2.setPower(sidePower + angleAdjustPower);
    ...
    public void scoreFoundation() {
        ...
        while (Math.abs(angleError) > 3 && opModeIsActive() && !isStopRequested()) {
            angles = imu.getAngularOrientation().toAxesReference(AxesReference.INTRINSIC).toAxesOrder(AxesOrder.ZYX);
            angleDouble = extraClasses.convertAngle(Double.parseDouble(formatAngle(angles.angleUnit, angles.firstAngle)));
            angleError = extraClasses.angleDistance(angleDouble, scoreAngle);
            if ((scoreAngle - angleDouble + 360) % 360 < 180) {
                motorPower = angleError / pValue;
            } else {
                motorPower = angleError / pValue * -1;
            }
            double minSpeed = .1;
        ...
        while(!scored && opModeIsActive() && !isStopRequested()) {
            ...
            if(!extraClasses.closeEnough(arm.getCurrentPosition(), goalArmPos, 30) && opModeIsActive() && !finishedMovementOverRobot) {
                ...
                if(armState == 0) {
                    ...
                    arm.setPower(.3);
                    if(arm.getCurrentPosition() <= -3600) {
                        arm.setPower(0);
    ...
    public void moveIntakeAndArm1() {
        ...
        clawServo.setPosition(servoOpenPos);
        if(blockPosition == 0) {
            leftIntakeServo.setPosition(.84);
    ...
    public void ThreadSleepUpdated(double waitTime) {
        ...
        while(!isDone && opModeIsActive() && !isStopRequested()) {
            ...
            if(timeDifference > waitTime && opModeIsActive() && !isStopRequested()) {
                isDone = true;
            }
        }
    }
